...
# =================================================================
# FORMULARIO PRINCIPAL: Crear/Editar el Protocolo Actual (Encabezado)
# =================================================================
class ProtocoloForm(forms.ModelForm):
    ...
    class Meta:
        model = ProtocolosDB
        fields = [
            'id', 'cod_dependencia', 'vigente', 'cod_protocolo', 'titulo', 
        ...
        widgets = {
            ...
            'cod_protocolo': forms.TextInput(attrs={'class': 'form-control', 'style': 'color: black;', 'placeholder': 'Código'}),
            'titulo': forms.Textarea(attrs={'rows': 3, 'class': 'form-control', 'style': 'color: black; height: 80px;', 'placeholder': 'Título del Protocolo'}),
    ...
    def clean_cod_protocolo(self):
        cod = self.cleaned_data.get('cod_protocolo')
...
# =================================================================
class ProtocolosHistoricoForm(forms.ModelForm):
    class Meta:
        model = ProtocolosHistoricoDB
        exclude = ['protocolo']
        widgets = {
            'cod_protocolo': forms.TextInput(attrs={'class': 'form-control', 'style': 'color: black;'}),
            'titulo': forms.Textarea(attrs={'rows': 3, 'class': 'form-control', 'style': 'color: black; height: 60px;', 'placeholder': 'Título del Protocolo en esta versión histórica'}),
            'fecha_elaboracion': forms.DateInput(attrs={'type': 'date', 'class': 'form-control'}, format='%Y-%m-%d'),
            'fecha_revision': forms.DateInput(attrs={'type': 'date', 'class': 'form-control'}, format='%Y-%m-%d'),
            'fecha_aprobacion': forms.DateInput(attrs={'type': 'date', 'class': 'form-control'}, format='%Y-%m-%d'),
...
class ProtocoloFilterForm(forms.Form):
    search_query = forms.CharField(
        required=False,
        label='Buscar Código/Descripción Protocolo', 
...
ProtocolosHistoricoFormSet = inlineformset_factory(
    ProtocolosDB,
    ProtocolosHistoricoDB,
    form=ProtocolosHistoricoForm,
